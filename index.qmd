...
<p align="right">
<a href="https://rf-capstone-project.github.io/project/slides.html" style="color: black;">Slides</a>
</p>
## Introduction
...
Hyperparameter tuning helps Random Forests create a balance between model complexity, computational efficiency, and generalization performance. One of the most influential parameters is the number of trees in the ensemble (*n_estimators*). Increasing this value usually reduces variance and improves overall stability, although the gains eventually level out while computational cost continues to rise. The maximum depth of each tree (*max_depth*) also plays an important role. Deeper trees can capture complex patterns in the data, but they may overfit when allowed to grow without constraint.
Another key parameter is the number of features considered at each split (*max_features*). Limiting this value prevents a small group of strong predictors from dominating every decision and helps reduce correlation across the forest. Also, parameters such as *min_samples_split* and *min_samples_leaf* prevent the model from creating branches based on very small sample sizes, making the model less sensitive to noise.  

The bootstrap setting (*bootstrap=True*) determines whether sampling occurs with replacement when generating training subsets for each tree. While this behavior is standard, not using bootstrapping can be useful in smaller datasets where more unique samples may be wanted. In practice, methods such as grid search and randomized search are commonly used to evaluate combinations of these hyperparameters, improving model accuracy and robustness in a systematic way [@GeeksforGeeks2024].
...
Initially we run a basic model with default parameters. In this stage of the project we used the *randomForest* and *rsample* R packages. The data was split into training and testing sets with an 80-20 ratio.  
